...
contract e2m is ERC721Enumerable, Ownable {
    ...
    // purchase price
    uint256 private _purchasePrice = 30000000000000000 wei;

    // baseURI for Metadata
    string private metadataURI = "https://minty.kola.app/e2m/metadata/";
    ...
    */
    ...
    function domainToTokenId(string calldata domainName) internal pure returns (uint256) {
    ...
    function tokenIdForDomain(string calldata domainName) external view returns (uint256) {
        uint256 tokenId = domainToTokenId(domainName);
        require(domainPurchases[tokenId],  "This domain hasn't been purchased.");
        return tokenId;
    }

    function tokenInfo(uint256 tokenId) external view returns (string memory domainName) {
    ...
    function mint(string calldata domainName) external payable returns (string memory) {
        uint256 slen = bytes(domainName).length;
        string memory name = domainName[:slen-4];
//        bytes32 label = keccak256(bytes(name));
//        uint256 tokenId = uint256(label);
...
//
//        uint256 tokenId = domainToTokenId(domainName);
//        require(!domainPurchases[tokenId],  "This domain has already been purchased.");
//        require(msg.sender == ens.ownerOf(tokenId),  "You are not the owner of this ENS domain, please input the name without '.eth'.");
//
//        _safeMint(msg.sender, tokenId);
//        tokenIdToDomain[tokenId] = domainName;
//        domainPurchases[tokenId] = true;
//
//        return tokenId;
    }
